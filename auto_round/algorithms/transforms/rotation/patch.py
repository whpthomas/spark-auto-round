...
def _lazy_qlinear_imports():
    # Lazy import for QuantLinear - only available when MXFP modules exist.
    from auto_round.export.export_to_autoround.qlinear_fp import QuantLinear, pack_fp4_to_uint8
    return QuantLinear, pack_fp4_to_uint8
from auto_round.wrapper import WrapperLinear, WrapperWALayer
...
def patch_quantlinear(w_transform) -> None:
    ...
    """
    QuantLinear, pack_fp4_to_uint8 = _lazy_qlinear_imports()
    ...
    def _pack_patched(
        ...
        if self.is_nv:
            ...
            from auto_round.data_type.nvfp import cast_to_fp4, get_reciprocal as _nvfp_reciprocal
            scaled_tensor = tensor.to(global_scale.dtype) * _nvfp_reciprocal(
                scales.reshape(tensor.shape[0], -1) * _nvfp_reciprocal(global_scale)
            )
