...
def quant_tensor_opt_rtn_sym(tensor, bits=4, group_size=-1, v=0, q_scale_thresh=1e-5, imatrix=None, **kwargs):
    ...
    def _imatrix_handle_zero(imatrix, tensor, bits):
        """Replace zero imatrix values with a small positive constant.

        Zero importance values can cause issues in scale computation.
        This replaces them with a small epsilon based on the tensor's magnitude.
        """
        eps = 1e-6
        zero_mask = imatrix == 0
        if zero_mask.any():
            # Use a small fraction of the tensor's mean absolute value
            fallback = tensor.abs().mean().item() * eps
            fallback = max(fallback, eps)
            imatrix = imatrix.clone()
            imatrix[zero_mask] = fallback
        return imatrix
