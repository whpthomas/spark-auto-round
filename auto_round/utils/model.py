...
# MXFP4 model support for loading pre-quantized models
...
def llm_load_model(
    ...
    **kwargs,
    ...
    if device == "meta":
        ...
        # Create model on CPU from config (no weights loaded)
        model = model_cls.from_config(config, trust_remote_code=trust_remote_code)

        # Move to meta device (all parameters become meta — zero memory)
        model = model.to(torch.device("meta"))

        # Store model path for later block loading
...
def is_gguf_model(model_path: Union[str, torch.nn.Module]) -> bool:
    """Check if the model path points to a GGUF file. GGUF is not supported in this build."""
    if isinstance(model_path, str) and os.path.isfile(model_path) and model_path.endswith(".gguf"):
        return True
...
# GGUF export functions not available in this build
# 
