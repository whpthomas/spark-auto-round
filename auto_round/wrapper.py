...
class WrapperLlamaNorm(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        eps = getattr(self.orig_layer, 'variance_epsilon', None) or getattr(self.orig_layer, 'eps', 1e-6)
...
NORM_MAPPING["Qwen3_5RMSNorm"] = WrapperLlamaNorm
    ...
    for n, m in block.named_modules():
        if type(m) in SUPPORTED_LAYER_TYPES:
            ...
            if "norm" in m.__class__.__name__.lower():
                if "Gated" in m.__class__.__name__:
                    pass  # gated norms take extra args, skip
                elif m.__class__.__name__ in NORM_MAPPING.keys():
