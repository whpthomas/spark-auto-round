...
def _make_report_csv(path, rows):
    """Create a quantization-report.csv with the given rows.

    Args:
        path: Directory to create the CSV in.
        rows: List of dicts with keys: block_name, cosine_sim, psnr_db, passed.
    """
    import csv

    path.mkdir(parents=True, exist_ok=True)
    csv_path = path / "quantization-report.csv"
    fieldnames = [
        "block_name", "cosine_sim", "psnr_db",
        "init_loss", "best_loss", "best_iter", "total_iters", "passed",
    ]
    with open(csv_path, "w", newline="") as f:
        ...
        for row in rows:
            ...
            full_row = {
                "init_loss": "", "best_loss": "", "best_iter": "0",
                "total_iters": "1000",
            }
            full_row.update(row)
...
class TestSelectLayersFromReport:
    def test_selects_failed_layers(self, tmp_path):
        """Default mode: select all layers with passed=false."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9980", "psnr_db": "48.0", "passed": "true"},
            {"block_name": "model.layers.2", "cosine_sim": "0.9850", "psnr_db": "42.0", "passed": "false"},
            {"block_name": "model.layers.3", "cosine_sim": "0.9920", "psnr_db": "44.0", "passed": "false"},
        ])
        result = select_layers_from_report(str(tmp_path))
        assert result == [2, 3]

    def test_all_passed_returns_empty(self, tmp_path):
        """When all layers pass, return empty list."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9998", "psnr_db": "54.0", "passed": "true"},
        ])
        result = select_layers_from_report(str(tmp_path))
        assert result == []

    def test_top_n_worst_cosine(self, tmp_path):
        """Top-N: pick N worst by cosine_sim."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9950", "psnr_db": "50.0", "passed": "true"},
            {"block_name": "model.layers.2", "cosine_sim": "0.9800", "psnr_db": "42.0", "passed": "false"},
            {"block_name": "model.layers.3", "cosine_sim": "0.9700", "psnr_db": "40.0", "passed": "false"},
            {"block_name": "model.layers.4", "cosine_sim": "0.9900", "psnr_db": "48.0", "passed": "true"},
        ])
        result = select_layers_from_report(str(tmp_path), top_n=2, metric="cosine")
        # Worst cosine: 0.9700 (layer 3), 0.9800 (layer 2)
        assert result == [2, 3]

    def test_top_n_worst_psnr(self, tmp_path):
        """Top-N: pick N worst by psnr_db."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9950", "psnr_db": "30.0", "passed": "false"},
            {"block_name": "model.layers.2", "cosine_sim": "0.9980", "psnr_db": "42.0", "passed": "true"},
            {"block_name": "model.layers.3", "cosine_sim": "0.9960", "psnr_db": "25.0", "passed": "false"},
        ])
        result = select_layers_from_report(str(tmp_path), top_n=1, metric="psnr")
        # Worst psnr: 25.0 (layer 3)
        assert result == [3]

    def test_threshold_cosine(self, tmp_path):
        """Threshold: select all layers below cosine threshold."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9950", "psnr_db": "50.0", "passed": "true"},
            {"block_name": "model.layers.2", "cosine_sim": "0.9850", "psnr_db": "42.0", "passed": "false"},
        ...
        result = select_layers_from_report(str(tmp_path), threshold=0.995, metric="cosine")
        # Below 0.995: layers 2 (0.985) and 3 (0.992)
        assert result == [2, 3]

    def test_threshold_psnr(self, tmp_path):
        """Threshold: select all layers below psnr threshold."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9950", "psnr_db": "50.0", "passed": "true"},
            {"block_name": "model.layers.2", "cosine_sim": "0.9980", "psnr_db": "45.0", "passed": "true"},
            {"block_name": "model.layers.3", "cosine_sim": "0.9960", "psnr_db": "43.0", "passed": "false"},
        ])
        result = select_layers_from_report(str(tmp_path), threshold=45.0, metric="psnr")
        # Below 45.0: layer 3 (43.0)
        assert result == [3]

    def test_missing_csv_raises(self, tmp_path):
        """Missing quantization-report.csv raises FileNotFoundError."""
        with pytest.raises(FileNotFoundError, match="Quantization report not found"):
            select_layers_from_report(str(tmp_path))
    ...
    def test_sorted_by_index(self, tmp_path):
        """Results are always sorted by layer index."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.5", "cosine_sim": "0.9800", "psnr_db": "42.0", "passed": "false"},
            {"block_name": "model.layers.2", "cosine_sim": "0.9700", "psnr_db": "40.0", "passed": "false"},
            {"block_name": "model.layers.8", "cosine_sim": "0.9600", "psnr_db": "38.0", "passed": "false"},
        ])
        result = select_layers_from_report(str(tmp_path))
        assert result == [2, 5, 8]

    def test_handles_language_model_prefix(self, tmp_path):
        """Handle both 'model.layers.X' and 'model.language_model.layers.X'."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.language_model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.language_model.layers.1", "cosine_sim": "0.9800", "psnr_db": "42.0", "passed": "false"},
        ])
        result = select_layers_from_report(str(tmp_path))
        assert result == [1]

    def test_top_n_exceeds_available(self, tmp_path):
        """Top-N larger than available layers returns all available."""
        _make_report_csv(tmp_path, [
            {"block_name": "model.layers.0", "cosine_sim": "0.9999", "psnr_db": "55.0", "passed": "true"},
            {"block_name": "model.layers.1", "cosine_sim": "0.9800", "psnr_db": "42.0", "passed": "false"},
        ])
        result = select_layers_from_report(str(tmp_path), top_n=10)
        # All layers returned (sorted by index)
        assert result == [0, 1]
