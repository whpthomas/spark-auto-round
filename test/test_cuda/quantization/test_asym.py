...
class TestAutoRoundAsym:
    ...
    def test_asym_group_size_with_tuning(self, group_size, tiny_opt_model_path):
        bits, sym = 4, True  # Using symmetric for auto_round format compatibility (AWQ not available)
        ...
        import os
        assert os.path.exists(quantized_model_path), f"Model not saved to {quantized_model_path}"
        assert os.path.exists(os.path.join(quantized_model_path, "config.json")), "config.json not found"

        # NOTE: Inference test disabled - requires gptqmodel>=2.0
    ...
    def test_asym_bits_with_tuning(self, bits, tiny_opt_model_path):
        group_size, sym = 128, True  # Using symmetric for auto_round format compatibility
        ar = AutoRound(tiny_opt_model_path, bits=bits, group_size=group_size, sym=sym, iters=1, seqlen=2, nsamples=1)
        _, quantized_model_path = ar.quantize_and_save(format="auto_round", output_dir=self.save_dir)
